...
/**
 * This class is a template for Ship in the Battleship game.
 * Each ship has length, name, coordinates and also maintains
 * its state like hits at its coordinates, sunk or not and whether
 * is arranged in ocean or lying idle in the fleet.
 * 
 * @author    Kritka Sahni
 * @author    Neha Upadhyay
 */
...
public class Ship {
	...
	/**
     	...
     	* match ship length or not. Also width needs to checked should be 1.
     	...
     	* @return    boolean Returns true coordinates meet conditions
     	*/
	public boolean isCoordinatesWithinLength( int x1, int y1, int x2, int y2 ) {
		int diffX = x2 - x1, diffY = y2 - y1;
		
		// ships should only vary in length ie, either x coordinates should be same 
		// or y coordinates should be same. They should have width = 1.
		if( x1 != x2 && y1 != y2 ){
			return false;
		}
		
		// coordinates must equal length
		if ( ( diffX < (length - 1) && diffY < (length - 1) ) || 
	...
	/**
     	* This method is used to determine if given coordinates overlap or touch the 
     	* current ship.
     	* 
     	* @param     x1      x-coordinate of starting point
     	* @param     y1      y-coordinate of starting point
	...
	public boolean isOverlap( int x1, int y1, int x2, int y2 ) {
		...
		//check adjacency
		//for each of coordinates of this ship, check if (x,y) is in its neighbourhood
		...
		if( this.x1 == this.x2) {
			...
			for( y = this.y1; y <= this.y2; y++ ) {
				for ( k = 0; k < 4; k++ ) {
					...
					if( ( x_co >= x1 && x_co <= x2) && ( y_co >= y1 && y_co <= y2) ) {
						return true;
					}
	...
	/**
     	* This method is used to determine if the ship is coordinates.
     	* 
