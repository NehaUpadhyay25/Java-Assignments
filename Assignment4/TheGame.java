...
public class TheGame {
	...
	* This method is used to play the Battleship game between two players.
     	...
     	*/
	public static void playGame( Player attacker, Player opponent ) {
		Player temp = null;
		int result = 1;
		Scanner sc = null;
		int k = 0;
		...
		while(  result != TheGame.OVER ) {
			...
			do {
				...
				result = opponent.reportHit( coords[0], coords[1] );
			} while ( result != TheGame.OVER && result != TheGame.MISS );
		
			// check if all the ships are sunk, then game over is displayed
			if ( result == TheGame.OVER ) {
	...
	* This method is the main method.
	...
	public static void main( String[] args ) {
		// create oceans for respective players
		Ocean aOcean   = new Ocean( minRow, minColumn, maxRow, maxColumn );
		Ocean bOcean   = new Ocean( minRow, minColumn, maxRow, maxColumn );
		
		// create players
        	Player playerA = new Player ( aOcean, "Player A" );
        	Player playerB = new Player ( bOcean, "Player B" );
        
        	// create fleet of player A 
        	if ( !playerA.createTheFleet() ) {
        		return;
        	}
        
        	// create fleet of player B
        	if( !playerB.createTheFleet() ) {
        		return;
        	}
